...
public class Main_ {
        ...
        for (int m = 1; m <= C; m++) {  //step1. 낚시왕 이동
            ...
            for (Pos s : sharkList) {
                if (m == s.getC()) {
                    hunt += s.getZ();
                    sharkList.remove(s);
                    //System.out.println("fisher:" + m + "열, remove" + s.getR() + "," + s.getC());
                    break;
                }
            }
            //System.out.println("상어잡고");
            //print();
    ...
    private static void print() {
        System.out.println("shark list");
        for (Pos s : sharkList) {
            System.out.println(s.getR() + "," + s.getC() + ", 방향:" + s.getD() + ", 속력:" + s.getS() + ", 크기:" + s.getZ());
        }
    }
    ...
    private static void move() {
        ...
        for (Pos s : sharkList) {
            ...
            //System.out.println("nr:" + s.getR() + ", nc:" + s.getC() + ", 방향:" + s.getD() + ", 속력:" + s.getS() + ", 크기:" + s.getZ());

            // 상어가 둘 이상 같은 위치에 있으면 작은상어 없애기
            if (map[s.getR()][s.getC()] != null) {
                if (map[s.getR()][s.getC()].getZ() < s.getZ()) {// 이미 담겨있는애가 큰지 내가 큰지
                    //sharkList.remove(map[s.getR()][s.getC()]);  //리스트에 담긴것도 지우고
